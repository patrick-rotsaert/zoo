...
TEST(PostgresqlQueryparametersTest, TimepointParameter)
{
	...
	EXPECT_EQ(get_one_query_parameter(conversion::string_to_time_point(tp)), tp);
}

TEST(PostgresqlQueryparametersTest, DateParameter)
{
	constexpr auto dt = "2023-06-25";
	EXPECT_EQ(get_one_query_parameter(conversion::string_to_date(dt)), dt);
}

TEST(PostgresqlQueryparametersTest, TimeofdayParameter)
{
	constexpr auto tm = "15:27:19";
	EXPECT_EQ(get_one_query_parameter(conversion::string_to_time_of_day(tm)), tm);
}

TEST(PostgresqlQueryparametersTest, BoostPtimeParameter)
{
	constexpr auto tp = "2023-06-25 15:27:19";
	EXPECT_EQ(get_one_query_parameter(conversion::string_to_boost_ptime(tp)), tp);
}

TEST(PostgresqlQueryparametersTest, BoostDateParameter)
{
	constexpr auto dt = "2023-06-25";
	EXPECT_EQ(get_one_query_parameter(conversion::string_to_boost_date(dt)), dt);
}

TEST(PostgresqlQueryparametersTest, BoostTimedurationParameter)
{
	constexpr auto tm = "15:27:19";
	EXPECT_EQ(get_one_query_parameter(conversion::string_to_boost_time_duration(tm)), tm);
}
