...
#include <zoo/squid/mysql/detail/queryparameters.h>
#include <zoo/squid/mysql/detail/query.h>
...
class query_parameter_binder final
{
	mysql_query                       q_;
...
public:
	template<typename T>
	explicit query_parameter_binder(const T& value)
	    : q_{ "SELECT :first" }
	    , p_{}
	    , qp_{}
	{
...
{
	query_parameter_binder qpb{ std::nullopt };
...
{
	{
		query_parameter_binder qpb{ true };
		const auto&            bind = qpb.bind();
	...
	{
		query_parameter_binder qpb{ false };
		const auto&            bind = qpb.bind();
...
TEST(MysqlQueryparametersTest, TimepointParameter)
{
	...
	query_parameter_binder qpb{ conversion::string_to_time_point(tp) };
	const auto&            bind = qpb.bind();
	...
	EXPECT_EQ(conversion::time_point_to_sql(out), tp);
}

TEST(MysqlQueryparametersTest, DateParameter)
{
	constexpr auto         dt = "2023-06-25";
	query_parameter_binder qpb{ conversion::string_to_date(dt) };
	const auto&            bind = qpb.bind();
	...
	date out;
	from_mysql_time(*reinterpret_cast<const MYSQL_TIME*>(bind.buffer), out);
	EXPECT_EQ(conversion::date_to_string(out), dt);
}

TEST(MysqlQueryparametersTest, TimeofdayParameter)
{
	constexpr auto         tm = "15:27:19.112233";
	query_parameter_binder qpb{ conversion::string_to_time_of_day(tm) };
	const auto&            bind = qpb.bind();
	...
	time_of_day out;
	from_mysql_time(*reinterpret_cast<const MYSQL_TIME*>(bind.buffer), out);
	EXPECT_EQ(conversion::time_of_day_to_string(out), tm);
}

TEST(MysqlQueryparametersTest, BoostPtimeParameter)
{
	constexpr auto         tp = "2023-06-25 15:27:19.123123";
	query_parameter_binder qpb{ conversion::string_to_boost_ptime(tp) };
	const auto&            bind = qpb.bind();
...
{
	constexpr auto         dt = "2023-06-25";
	query_parameter_binder qpb{ conversion::string_to_boost_date(dt) };
	const auto&            bind = qpb.bind();
...
TEST(MysqlQueryparametersTest, BoostTimedurationParameter)
{
	...
	query_parameter_binder qpb{ conversion::string_to_boost_time_duration(tm) };
	const auto&            bind = qpb.bind();
