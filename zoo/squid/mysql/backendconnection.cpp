...
#include "zoo/squid/mysql/statement.h"
#include "zoo/squid/mysql/error.h"
...
std::string parse_parameter_value(std::string_view& in)
{
	...
	else if (in.front() == '"')
	{
		...
		for (;;)
		{
			if (in.empty())
			{
				ZOO_THROW_EXCEPTION(error{ ERROR_MESSAGE_MALFORMED + ": unterminated quoted value" });
			}
			...
			{
				ZOO_THROW_EXCEPTION(error{ ERROR_MESSAGE_MALFORMED + ": only `\"' and `\\' characters may be escaped" });
			}
			else
			{
				value.append({ c });
...
struct connection_parameters final
{
...
connection_parameters parse_connection_string(std::string_view in)
{
	...
	for (;;)
	{
		...
		if (name.empty())
		{
			ZOO_THROW_EXCEPTION(error{ ERROR_MESSAGE_MALFORMED + ": parameter name expected" });
		}
		...
		{
			ZOO_THROW_EXCEPTION(error{ ERROR_MESSAGE_MALFORMED + ": '=' expected after parameter name '" + std::string{ name } + "'" });
		}
		...
		else if (name == "port")
		{
			...
			if (!end || *end)
			{
				ZOO_THROW_EXCEPTION(error{ "Invalid connection string. Value for `port' parameter is not a valid integer" });
			}
		...
		{
			ZOO_THROW_EXCEPTION(error{ "Invalid connection string. Unknown parameter name '" + std::string{ name } + "'" });
		}
	}
...
std::shared_ptr<MYSQL> connect_database(std::string_view connection_info)
{
	...
	if (!handle)
	{
		ZOO_THROW_EXCEPTION(error{ "mysql_init: out of memory" });
	}
	...
	if (params.charset)
	{
		...
		{
			ZOO_THROW_EXCEPTION(error("mysql_options(MYSQL_SET_CHARSET_NAME) failed", *handle));
		}
	}

	if (params.sslca)
	{
		if (mysql_options(handle.get(), MYSQL_OPT_SSL_CA, params.sslca->c_str()))
		{
			ZOO_THROW_EXCEPTION(error("mysql_options(MYSQL_OPT_SSL_CA) failed", *handle));
		}
	}

	if (params.sslcert)
	{
		if (mysql_options(handle.get(), MYSQL_OPT_SSL_CERT, params.sslcert->c_str()))
		{
			ZOO_THROW_EXCEPTION(error("mysql_options(MYSQL_OPT_SSL_CERT) failed", *handle));
		}
	}

	if (params.sslkey)
	{
		if (mysql_options(handle.get(), MYSQL_OPT_SSL_KEY, params.sslkey->c_str()))
		{
			ZOO_THROW_EXCEPTION(error("mysql_options(MYSQL_OPT_SSL_KEY) failed", *handle));
		}
	}
	...
	{
		ZOO_THROW_EXCEPTION(error("mysql_real_connect failed", *handle));
	}
