...
class query_results::column final
{
		    ...
		    [&](auto&& arg) {
			    ...
			    else if constexpr (std::is_same_v<T, std::string> || std::is_same_v<T, byte_string>)
			    {
				    ...
				    if (this->bind_->length_value > 0ul)
				    {
					    ...
					    {
						    ZOO_THROW_EXCEPTION(error{ "mysql_stmt_fetch_column failed", *this->statement_ });
					    }
				    }
			    }
    ...
    , columns_{}
{
	...
	if (this->meta_)
	{
		...
		if (!this->fields_)
		{
			ZOO_THROW_EXCEPTION(error{ "mysql_fetch_fields returned a nullptr", *this->statement_ });
		}
    ...
    : query_results{ statement }
{
	...
	{
		ZOO_THROW_EXCEPTION(error{ fmt::format("Cannot fetch {result_count} columns from a result set with {field_count} column{ord}",
	...
	{
		ZOO_THROW_EXCEPTION(error{ "mysql_stmt_bind_result failed", *this->statement_ });
	}
}
    ...
    : query_results{ statement }
{
	...
	for (std::size_t i = 0, end = this->field_count_; i < end; ++i)
	{
		...
		if (name == nullptr)
		{
			ZOO_THROW_EXCEPTION(error{ "mysql_fetch_fields returned a field with a null pointer for a name" });
		}
...
std::string_view query_results::field_name(std::size_t index) const
{
	...
	{
		ZOO_THROW_EXCEPTION(error{ "Field index out of bounds" });
	}
}
...
bool query_results::fetch()
{
	...
	case MYSQL_NO_DATA:
		return false;
	default:
		ZOO_THROW_EXCEPTION(error{ "mysql_stmt_fetch failed", *this->statement_ });
	}
