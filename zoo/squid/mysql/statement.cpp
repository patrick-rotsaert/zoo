...
#include "zoo/squid/mysql/detail/queryresults.h"
...
std::shared_ptr<MYSQL_STMT> prepare_statement(MYSQL& connection, const std::string& query)
{
	...
	if (!stmt)
	{
		ZOO_THROW_EXCEPTION(error{ "mysql_stmt_init failed", connection });
	}

	if (0 != mysql_stmt_prepare(stmt.get(), query.c_str(), static_cast<unsigned long>(query.length())))
	{
		ZOO_THROW_EXCEPTION(error{ "mysql_stmt_prepare failed", connection });
	}
...
public:
	...
	void execute(const std::map<std::string, parameter>& parameters, const ResultsContainer& results)
	{
		...
		{
			ZOO_THROW_EXCEPTION(error{ "mysql_stmt_execute failed", *this->statement_ });
		}
		...
		{
			ZOO_THROW_EXCEPTION(error{ "mysql_stmt_store_result failed", *this->statement_ });
		}
	}
	...
	MYSQL_STMT& handle() const
	{
		if (!this->statement_)
		{
			ZOO_THROW_EXCEPTION(error{ "Statement has not been executed" });
		}
		else
		{
...
/*static*/ void statement::execute(MYSQL& connection, std::string_view query)
{
	if (0 != mysql_real_query(&connection, query.data(), static_cast<unsigned long>(query.length())))
	{
